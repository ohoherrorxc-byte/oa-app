...
const __STICKY_TABLE = {
  ...
  setHeadWidth1: stickyDebounce((_this, data) => {
    ...
    tableStickyObj[uid].headerRect.top =  _this.getPosY(el, binding.value.parent) 
  ...
  setEl(el){
    return el.className.includes('el-table') ? el : el.querySelectorAll('.el-table')[0]
  }
...
Vue.directive('sticky', {
  ...
  inserted(el, binding, vnode) {
    if (binding.value === false) return false
    el = __STICKY_TABLE.setEl(el)
