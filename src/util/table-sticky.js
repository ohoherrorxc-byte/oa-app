// https://blog.csdn.net/qq_43526066/article/details/129062116 参考文档
...
export default {
  mounted() {
   if(this.intercept()) return false
    // this.containerDom = document.getElementsByClassName('main-container')
    this.containerDom = document.querySelectorAll('.avue-main')
    this.clearListener()
  ...
  activated() {
    ...
    timer = setTimeout(() => {
      ...
      if (container && container[0].scrollTop > 0) {
  ...
  methods: {
    intercept(){
      let elTable = document.getElementsByClassName('el-table')
      if(elTable.length != 1) return true
      if(elTable[0].classList.contains('no-sticky')) return true
      if(!document.querySelectorAll('.el-table__body-wrapper tbody tr')[0]) return true
    },
    activatedReload() {
      window.addEventListener('resize', this.resizeChange)
      let timer = setTimeout(() => {
    ...
    async initFixedHeader() {
      if (this.parent) {
        ...
        if (this.parentDom && this.parentDom.length !== 0 && this.parentDom[0] && this.parentDom[0].querySelector('.el-table__header-wrapper')) {
          ...
          this.scrollDom = document.querySelector('#avue-view')
    ...
    updateFixedRight() {
      ...
      if (dom && dom.classList.contains('fixed') && this.fixedRightDom && this.fixedRightDom[0]) {
        let timer = setTimeout(() => {
          this.setFixedStyle({
    ...
    setScrollXWidth() {
      let timer = setTimeout(() => {
        ...
        if (this.parentDom.length == 0 || this.tableDom.length == 0) return false
    ...
    updateWidth() {
      if (!this.parentDom) this.parentDom = document.getElementsByClassName(this.parent)
      if (!this.parentDom[0]) return false
      ...
      this.$nextTick(() => {
        ...
        if (obj.height != this.tablexy.height) {
          ...
          if (dom.classList.contains('fixed')) {
            let timer = setTimeout(() => {
              if(this.parentDom[0].getElementsByClassName('el-table__fixed-body-wrapper')[0]) {
    ...
    scrollEvent: rafThrottle(async function (e) {
      ...
      if (scrollTop >= height / 2 + top) {
        ...
        dom.style.zIndex = this.styleZIndex
        dom.style.top = this.styleTop + 'px'
    ...
    clearFixedStyle() {
      ...
      if (dom && dom.classList.contains('fixed')) {
        if (fixedRightHeaderDom) {
