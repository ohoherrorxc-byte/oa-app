...
export default {
  ...
  data() {
    let checkReceiptAmount = (rule, value, callback) => {
      if (isNaN(value)) {
        ...
        callback(new Error("本次收款金额不能大于剩余待收款金额"));
      } else {
        callback();
      }
    };
  ...
  methods: {
    ...
    reloadFormAndTable() {
      let orderNumber = this.$route.query.orderIdTbox;
    ...
    updateLicense(form, done, value) {
      let licenseForm = {};
      licenseForm.vinList = "";
      licenseForm.orderIdTbox = form.orderNumber;
      licenseForm.statusTbox = (() => {
        if (value) return value;
        return this.newOrderStatus;
      })();

      let { vinList, orderIdTbox, statusTbox } = licenseForm;
      updateBatch(vinList, statusTbox, orderIdTbox, "tbox").then(
        (res) => {
          if (res.data.success) {
            /*this.$message({
            type: "success",
    ...
    addPayment(form, done) {
      ...
      paymentForm.orderType = 0; //0:Tbox收款
    ...
    backToList() {
      ...
      this.$router.push({ path: "/saic/tboxOrder" });
    },
  },
};
</script>
