  ...
  <basic-container>
    <el-card>
      <el-form :inline="true" :model="query" class="demo-form-inline">
        ...
        <!--          <el-select v-model="query.supplierNameWeather" placeholder="全部" style="width: 90%" clearable>-->
        ...
        <el-form-item label="车型是否统计：">
          <el-select
            v-model="query.modelNeedStatisticsWeather"
            placeholder="请选择"
            style="width: 90%"
            clearable
          >
        ...
        <el-form-item label="收款状态：">
          <el-select v-model="query.statusTbox" style="width: 90%" clearable>
            <el-option
              v-for="item in collectionStatuss"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            >
    ...
    <el-card>
      ...
      <el-row>
        ...
        >
          ...
          <!--          </el-table-column>-->
          <!--          <el-table-column prop="supplierNameWeather" align="center" label="供应商">-->
          <!--          </el-table-column>-->
          <el-table-column
            prop="countryReginCode"
            align="center"
            label="国家地区"
            v-bind:formatter="countryCodesFormat"
          >
          </el-table-column>
          ...
          <el-table-column
            ...
            label="付款订单号"
            width="100"
            v-bind:formatter="formatterData"
          >
          </el-table-column>
          <el-table-column
            prop="modelTypeName"
          ...
          <el-table-column align="center" label="车辆是否统计">
              ...
              <div>
                <div v-if="scope.row.isTestVehicle == 1">
                  <el-tooltip content="工程车" placement="top">
                    <el-switch
                      disabled="true"
                      :value="scope.row.isTestVehicle"
                    ></el-switch>
                  </el-tooltip>
                </div>
                <div
                  v-if="
                    scope.row.isTestVehicle == 0 &&
                    scope.row.needStatisticsWeather == 0 &&
                    (scope.row.statusWeather == 1 ||
                      scope.row.statusWeather == 2)
                  "
                >
                  <el-switch
                    :value="!!scope.row.needStatisticsWeather"
                ...
                <div
                  v-if="
                    scope.row.isTestVehicle == 0 &&
                    scope.row.needStatisticsWeather == 1 &&
                    (scope.row.statusWeather == 1 ||
                      scope.row.statusWeather == 2)
                  "
                >
                ...
                >
                  <el-switch
                    v-model="!!scope.row.needStatisticsWeather"
                    disabled
                  />
                </div>
              </div>
      ...
      <el-row>
        <el-col :span="24">
          <div class="tool-box">
            <p style="color: red" v-show="isShow">
              * 查询结果中车型存在多个预设供应商，请确认查询结果是否正确
            </p>
          </div>
        </el-col>
      </el-row>
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="needStatisticsCancel"
          >取 消</el-button
        >
...
export default {
  ...
  created() {
    ...
    // this.query.supplierNameWeather = this.$route.query.supplierName;
    // this.query.statusWeather = this.$route.query.orderStatus;
  ...
  activated() {
    ...
    //TODO
    // this.onLoad(this.page, this.query);
    if (this.query.orderIdWeather) {
      this.query.needStatisticsWeather = this.query.modelNeedStatisticsWeather =
        null;
      this.onLoad(this.page, this.query);
    }
  },
  ...
  methods: {
    ...
    booleanFormatter(row, column) {
      return findByvalue(
        constData.booleanValue,
        row.modelNeedStatisticsWeather
      );
    },
