  ...
  <basic-container>
    <el-card>
      <el-form :inline="true" ref="query" :model="query" class="demo-form-inline">
        <el-form-item :label="$t('partsMaint.partsNo')">
          <el-input v-model.trim="query.partsNo" size="mini" clearable />
        </el-form-item>
        <el-form-item :label="$t('partsMaint.partsName')">
          <el-input v-model.trim="query.partsName" size="mini" clearable />
        </el-form-item>
        <el-form-item :label="$t('partsMaint.contract')">
          <el-input v-model.trim="query.bomPurchaseContractId" size="mini" clearable />
        </el-form-item>
        <el-form-item :label="$t('partsMaint.supplier')">
          <template>
            <el-select size="mini" v-model="query.supplierNo" filterable style="width: 90%" clearable>
        ...
        <el-form-item>
          <el-button type="primary" v-on:click="searchChange" size="mini" icon="el-icon-search">{{
            $t('commonContent.search') }}</el-button>
          <el-button type="primary" v-on:click="ResetChange" size="mini" icon="el-icon-refresh">{{
            $t('commonContent.refresh') }}</el-button>
          <el-upload style="display: inline-block; margin-left: 10px" action="#" :before-upload="beforeUpload" :limit="1">
            <el-button icon="el-icon-upload" size="mini" type="primary">{{ $t('commonContent.import') }}</el-button>
          </el-upload>
          <el-button icon="el-icon-download" size="mini" type="primary" style="margin-left: 10px"
            @click="handleExport('', 'create')">{{ $t('commonContent.export') }}</el-button>
          <el-button icon="el-icon-plus" size="mini" type="primary" style="margin-left: 10px"
            @click="handleAddOrUpdateOrDetail('', 'create')">{{ $t('commonContent.add') }}</el-button>
          <el-button icon="el-icon-delete" size="mini" type="danger" style="margin-left: 10px" plain
            @click="handleDeleteData('', 'littleDelete')">{{ $t('commonContent.delete') }}</el-button>

          <el-button type="primary" v-on:click="copyRow" size="mini" icon="el-icon-document-copy">{{
            $t('commonContent.copy') }}</el-button>
        </el-form-item>
      </el-form>
    </el-card>
    <el-tabs v-model="query.type" @tab-click="handleTypeChange">
      <el-tab-pane :label="$t('partsMaint.allParts')" name="1"></el-tab-pane>
    ...
    <el-table :data="tableData" style="width: 100%; margin-top: 20px" stripe align="center" border
      ...
      <el-table-column prop="partsNo" :label="$t('partsMaint.partsNo')" align="center" width="160"
        show-overflow-tooltip />
      <el-table-column prop="partsName" :label="$t('partsMaint.partsName')" align="left" width="250"
        show-overflow-tooltip />
      <el-table-column v-if="canShowPrice()" prop="unitPriceCny" :label="$t('partsMaint.unitPriceCny')" align="center" />
      ...
      <el-table-column prop="parentFunctionName" :label="$t('partsMaint.parentFunctionName')" align="center" width="100"
        show-overflow-tooltip />
      <el-table-column prop="childrenFunctionName" :label="$t('partsMaint.childrenFunctionName')" align="center"
        width="100" show-overflow-tooltip />
      <!-- <el-table-column prop="supplierName" label="所属供应商NO" align="center" /> -->
      <el-table-column prop="createUserName" :label="$t('partsMaint.createUserName')" align="center" />
      <el-table-column fixed="right" :label="$t('commonContent.operation')" width="250" align="center">
        <template slot-scope="scope">
          <el-button @click="handleAddOrUpdateOrDetail(scope.row, 'detail')" type="text" icon="el-icon-view">{{
          ...
          <el-button @click="handleAddOrUpdateOrDetail(scope.row, 'edit')" v-if="statusNot134(scope.row)" type="text"
            icon="el-icon-edit">{{ $t('commonContent.edit') }}</el-button>
          <el-button @click="handleDeleteData(scope.row, 'delete')" v-if="statusNot134(scope.row)" type="text"
            icon="el-icon-delete">{{ $t('commonContent.delete') }}</el-button>
          <el-button v-if="!scope.row.partsPriceStatus&&canSend(scope.row)" icon="el-icon-message" type="text" style="margin-left: 10px" plain
            @click="sendToPurchase(scope.row)">{{ $t('partsMaint.sendPurchase') }}</el-button>
            <el-button type="text" style="color:green" v-if="scope.row.partsPriceStatus===1&&canSend(scope.row)">已发送至采购</el-button>
            <el-button type="text" style="color:green" v-if="scope.row.partsPriceStatus===2&&canSend(scope.row)">已发送至采购,已归档</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!--分页-->
    ...
    <MaintainRecords v-if="maintainRecords" :params="{ id: rowId, type: 'PARTS_MAINT' }" @close="maintainRecords = false">
    </MaintainRecords>
  </basic-container>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      ExchangeRate: "",
      editFormRules: {
        unitPrice: [{ required: true, message: this.$t('partsMaint.unitPriceRequired') }],
        effectiveTime: [
          { required: true, message: this.$t('partsMaint.effectiveTimeRequired') },
        ],
        failureTime: [
          { required: true, message: this.$t('partsMaint.failureTimeRequired') },
        ],
      },
    };
  },
  ...
  methods: {
    canShowPrice() {
      return this.userInfo.content.account === 'shenyanxia' || this.userInfo.content.account === 'zhanpeng' || this.userInfo.content.account === 'wuzhizhen'
    },
    canSend(row) {
      return row && row.partsNo && this.userInfo.content.user_id === row.createUser && (this.userInfo.content.account === "jizhonghua" || this.userInfo.content.createUserName === "zhouxiangyu"|| this.userInfo.content.createUserName === "niuyinuo")
    },
    async sendToPurchase(row) {
      this.$confirm('是否确定发送流程到采购人员?', '提示', {
        ...
        let res = await PartsPriceMaintenanceApi.startBranchPartsPrice(row.id,"main")
    ...
    async copyRow() {
      ...
      let res = await getPartsCopy({ ids: ids.toString(), createUser: this.userInfo.content.user_id })
      console.log(res)
    ...
    handleAddOrUpdateOrDetail(row, key) {
      let path = row
        ? `/partsMaintAdd?id=${row.id}&key=${key}`
        : "/partsMaintAdd";
