          ...
          <el-col :span="12">
            <el-form-item prop="bomIds">
              ...
              <el-select :disabled="isShowDetail" style="width: 100%" v-model="editForm.bomIds" clearable filterable
                multiple @change="selectBom" popper-class="bom-select-dropdown" popper-append-to-body>
                <el-option v-for="item in bomList" :key="item.id" :label="item.bomName" :value="item.id">
                                  <div style="display:flex;flex-direction:column;line-height:1.2;padding:4px 0;">
                                    <div style="font-weight:600">{{ item.bomName }}</div>
                                    <div style="font-size:12px;color:var(--el-text-color-secondary)">BOM类型: {{ item.bomType }} · 国家: {{ item.countryRegin }}</div>
                                    <div style="font-size:12px;color:var(--el-text-color-secondary);white-space:normal;word-break:break-word;overflow-wrap:break-word">包含零件: {{ item.partNames }}</div>
                                  </div>
                </el-option>
              </el-select>
            </el-form-item>
    ...
    return {
      ...
      option: {
        ...
        column: [
          ...
          {
                      type: "select",
                      label: this.$t('vehicleModelManage.statusName'),
                      prop: "statusName",
                      clearable: true,
                      dicData: [
                        { label: "未关联", value: "未关联" },
...
.bom-select-dropdown .el-select-dropdown__wrap {
  max-width: 360px !important;
}
</style>
