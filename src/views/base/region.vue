<template>
  <el-row>
    ...
    <el-col :span="15">
      <basic-container>
        <el-button-group>
          <el-button v-if="permission.region_add" type="primary"  icon="el-icon-circle-plus-outline" @click="addChildren">新增下级</el-button>
          <el-button v-if="permission.region_delete" type="primary"  icon="el-icon-delete" @click="handleDelete">删除</el-button>
          <el-button v-if="permission.region_import" type="primary"  icon="el-icon-upload2" @click="handleImport">导入</el-button>
          <el-button v-if="permission.region_export" type="primary"  icon="el-icon-download" @click="handleExport">导出</el-button>
          <el-button v-if="permission.region_debug" type="primary"  icon="el-icon-video-play" @click="handleDebug">调试</el-button>
        </el-button-group>
      </basic-container>
      <basic-container>
        ...
        regionOption: {
          ...
          column: [
            ...
            },
            {
              ...
              prop: "regionLevel",
              type: "radio",
              dicUrl: "/api/oa-system/dict/dictionary?code=region",
              props: {
                label: "dictValue",
                value: "dictKey"
              },
          ...
          column: [
            {
              ...
              action: "/api/oa-system/region/import-region"
            },
            {
              label: "数据覆盖",
              prop: "isCovered",
          ...
          column: [
            {
              ...
              dicUrl: '/api/oa-system/region/select',
              span: 24,
            },
            {
              label: '地市',
              ...
              dicFlag: false,
              dicUrl: '/api/oa-system/region/select?code={{key}}',
              span: 24,
            },
            {
              label: '区县',
    ...
    watch: {
      ...
      'excelForm.isCovered'() {
        if (this.excelForm.isCovered !== '') {
          ...
          column.action = `/api/oa-system/region/import-region?isCovered=${this.excelForm.isCovered}`;
        }
      }
    },
    computed: {
    ...
    methods: {
      ...
      handleExport() {
        this.$confirm("是否导出行政区划数据?", "提示", {
          ...
          window.open(`/api/oa-system/region/export-region?${this.website.tokenHeader}=${getToken()}`);
        });
      },
      handleTemplate() {
        window.open(`/api/oa-system/region/export-template?${this.website.tokenHeader}=${getToken()}`);
      },
    }
  };
</script>
