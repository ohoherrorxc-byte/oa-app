...
export const mixins = {
    ...
    methods: {
        ...
        getSubjetRow(item,row, index) {
            ...
            if (!item) return
            row.costCenterName = item.costName
            row.costCenterId = item.costCenterId
            row.subjectName = `${item.subjectName}`//${item.parentSubjectName}/
            ...
            row.availableBudget = item.availableBudget
            ...
            this.form.bomPurchaseOrderDetailList.forEach(ele => {
                ele.costCenterName = row.costCenterName
                ele.costCenterId = row.costCenterId
                ele.subjectName = row.subjectName
                ele.subjectCode = row.subjectCode
                ele.subjectNo = row.subjectNo
                ele.subjectId = row.subjectId
                ele.avaliableFee = row.avaliableFee
                ele.costedFee = row.costedFee
                ele.underApprovalFee = row.underApprovalFee
                ele.budgetInfo = row.budgetInfo
            });
        },
        getCostItem(row, index) {
            let obj = this.costList.find(ele => {
                return ele.id === row.costCenterId
            })
            if(!obj) return
            row.costCenterId = obj.id
            row.costCenterName = obj.costName
        ...
        async getFeeDetail(row, index) {
            if (row.costCenterId && row.subjectId) {
                ...
                if (res.data.code === 0) {
                    ...
                    row.availableBudget = data.availableBudget
                    row.expensesIncurred = data.expensesIncurred 
                    row.expensesUnderReview = data.expensesUnderReview
                    row.subjectCode = data.subjectCode
            ...
            // form.bomPurchaseOrderDetailList
        },
