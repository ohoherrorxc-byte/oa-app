<template>
    <div class=" mb25 work-center" :style="styleItem">
        <el-row type="flex" justify="space-between" class="title-top">
            <el-col  @click="navigateToList(item.label)">{{$t(`newsBulletin.${item.label}`)}}</el-col>
            <el-col class="text-r">
                <i  @click="navigateToList(item.label)" class="el-icon-arrow-right"></i>
            </el-col>
        </el-row>
      <div class="content1">
...
export default {
    ...
    computed: {
        ...
        styleItem() {
            ...
            return {
                "margin-right": this.item.label !== '公司制度' ? "25px" : ""
            }
        }
    },
    methods: {
        async GetUser() {
            ...
            if (res.data.data&&res.data.data.supplierCompanyId != null) {
                //非正式员工
...
::v-deep ep.el-carousel__indicators--horizontal {
    display: none;
}
