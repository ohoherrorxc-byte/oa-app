  ...
  <basic-container>
    <avue-crud
      ...
      <template slot="menuLeft">
        <div class="btn">
          <upload @handleRequest="handleRequest" :showTip="false" class="mx10" url="/api/oa-portal/flowTable/importBusinessHospitality"></upload>
...
<script>
import OriginalStatementApi from "@/api/flowTable/businessHospitality/index";
...
export default {
  ...
  methods: {
    ...
    view(row){
      this.$router.push(`/businessHospitality?businessId=${row.id}&taskId=${row.taskId}&type=haveDone&processIsFinished=${row.processIsFinished}&status=${row.status}`)
    },
    async handleExport() {
      // 构建与 onLoad 一致的请求参数
            let obj = {
    ...
    async confirmUpdatePayStatusForSelection() {
            ...
            try {
              ...
              // 默认使用单条接口并发执行
              await Promise.all(ids.map(id => OriginalStatementApi.updateBusinessActualPayStatus({ id })));
