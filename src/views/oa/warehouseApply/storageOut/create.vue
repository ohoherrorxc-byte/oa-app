<template>
    <basic-container>
        ...
        <el-table :data="form.detailList" border class="mt10"  row-key="id"   :tree-props="{children: 'childList',hasChildren: 'hasChildren'}">
            <el-table-column prop="partsNo" :label="$t('parts.number')"  width="150"></el-table-column>
            <el-table-column prop="partsName" :label="$t('parts.name')" show-overflow-tooltip width="150"></el-table-column>
            <el-table-column prop="stockInNo" :label="$t('stockIn.number')"></el-table-column>
            <el-table-column prop="intoTime" :label="$t('stockIn.date')" width="150"></el-table-column>
...
export default {
    ...
    methods: {
        async getDetail() {
            let res = await WarehouseApplyApi.financeStockOutApplyDetails(this.businessId);
