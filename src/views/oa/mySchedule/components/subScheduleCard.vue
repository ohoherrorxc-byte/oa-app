<template>
    <div v-if="this.list.length>0">
        <div v-for="item in list" class="mb20 sub-item" :style=[styleLine(item)]>
            <div @click="toEdit(item)">
                <div class="pb5 sub-title">
                    <span>{{ item.schedule }}</span>
                </div>
                <div class="text-desc" :style=[styleDelay(item)]>
                   <span> {{ item.formatEndTime }}</span>
                   <img class="ml10" v-if="item.remindTime" width="16" src="@/assets/svg/meeting/mention.svg" alt="">
                </div>
            </div>
            <div class="button-row">
                <svg @click="changeChoose(item)" t="1695014481554" class="icon" viewBox="0 0 1024 1024" version="1.1"
                    xmlns="http://www.w3.org/2000/svg" p-id="6792" width="20" height="20">
                    <path
                        ...
                        :fill="item.fillChoose" p-id="6793"></path>
                </svg>
                <svg v-if="!isToday" @click="changeCollect(item)" t="1695015485595" class="icon ml10"
                    viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="5502" width="20"
                    height="20">
                    <path
                        ...
                        :fill="item.fillCollect" p-id="5503"></path>
                </svg>
            </div>
        </div>
        <editSchedule ref="editSchedule" @getList="getList" :row="curScheduleRow"></editSchedule>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        async getList() {
            ...
            let obj = {
                scheduleStatus: null,
                todayStatus: this.listType === 'today' ? 1 : null,
                types: this.listType==='toMe'?[1]:this.listType==='fromMe'?[0]:null,
                importantStatus: this.listType === 'important' ? 1 : null
            }
            //todayStatus
            let res = await scheduleApi.getTabList(obj)
            if (res.data.code === 0) {
                this.list = res.data.data
                if (this.list.length !== 0) {
                    this.$emit('dealNotEmpty')
                }
            }
            this.initList()
        },
        initList() {
            this.list.map(ele => {
                console.log(yearMonthDayWeek(ele.endTime))
                let tempColor = ele.scheduleStatus ? "#5E91DE" : "#E9E9E9"
                this.$set(ele, 'fillChoose', tempColor)
                let tempCollect = ele.importantStatus ? "#FFC076" : "#E9E9E9"
                this.$set(ele, 'fillCollect', tempCollect)
                this.$set(ele, 'formatEndTime', yearMonthDayWeek(ele.endTime))
                let isDelay = new Date(ele.endTime) < new Date()
                this.$set(ele, 'isDelay', isDelay)
            })
        },
        ...
        async changeChoose(item) {
            let obj = {
                id: item.id,
                scheduleStatus: item.scheduleStatus ? 0 : 1
            }
        ...
        toEdit(item) {
            ...
            //确保每次打开抽屉都请求接口
            this.curScheduleRow = null
            this.curScheduleRow = item
        },
