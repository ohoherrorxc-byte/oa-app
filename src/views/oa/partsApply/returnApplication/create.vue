<template>
  <basic-container>
    ...
    <el-form :disabled="disabled" ref="form" :rules="rules" :model="form" label-width="100px">
      <el-form-item :label="$t('return.instructions')" prop="returnRemark" style="width: 100%">
        <el-input type="textarea" v-model="form.returnRemark"></el-input>
      </el-form-item>
        ...
        >
          ...
          <el-table-column property="partName" :label="$t('parts.name')" align="center" width="180">
            <template slot-scope="scope">
              <el-form-item
                style="width: 100%;"
                :prop="'partBasicDtoList.' + scope.$index + '.partName'"
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        returnRemark: [
          { required: true, message: this.$t('return.returnRemarkRequired'), trigger: 'blur' }
        ],
  ...
  methods: {
    ...
    async getDetail() {
      ...
      let res = await PartsApplyApi.returnDetails(this.businessId);
