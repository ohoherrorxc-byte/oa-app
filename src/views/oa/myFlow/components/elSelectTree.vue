    ...
    <el-select class="contentSelect" :disabled="disabled" ref="treeSelect" :value="value" v-model="valueName"
        ...
        <el-option :value="valueName" class="options">
            <el-tree id="tree-option" :default-expand-all="false" :filter-node-method="filterNode" :show-checkbox="multiple"
                ...
                @node-click.self="handleNodeClick" :load="loadNode" lazy>
...
<script>
import {startList} from '@/api/myFlow/index'
export default {
    name: "el-tree-select",
    props: {
        // 配置项
    ...
    methods: {
      ...
      async  loadNode(node, resolve) {
            if (node.level > 1&&node.data.dictKey) return resolve([]);
          

            setTimeout(async() => {
                let res  =  await startList({mode:1,current:1,size:100,category:`flow_${node.data.dictKey}`})
                let data = res.data.data.records||[]
                data.map(ele=>{
                    ele.dictValue = ele.name
                    ele.leaf = true
                })
                resolve(data);
            }, 500);
