  ...
  <basic-container>
    ...
    <el-form :disabled="disabled" ref="form" :rules="rules" :model="form" label-width="120px">
      ...
      <el-form-item :label="$t('purchase.exchangeRate')" prop="exchangeRate">
        <el-input @blur="getOriginalAmountToTal()" v-model="form.exchangeRate" clearable></el-input>
      </el-form-item>
      <el-form-item :label="$t('purchase.totalAmountOriginal')" prop="assetNumber">
      ...
      <el-form-item :label="$t('purchase.applyDesc')" style="width: 96%" prop="applyDesc">
        <el-input type="textarea" v-model="form.applyDesc"></el-input>
      </el-form-item>
      <el-form-item :label="$t('purchase.singleSupplierReason')" style="width: 96%">
        <el-input type="textarea" v-model="form.singleSupplierReason"></el-input>
      </el-form-item>
      ...
      <el-form-item :label="$t('purchase.purchaseType')" prop="purchaseType">
        <el-select v-model="form.purchaseType" clearable>
          <el-option v-for="item in purchaseTypeList" :key="item.dictKey" :label="item.dictValue"
            :value="item.dictKey"></el-option>
        </el-select>
      </el-form-item>
      ...
      <el-form-item :label="$t('purchase.expectedPurchaseDate')" prop="expectedPurchaseDate">
        <el-date-picker style="width: 100%" value-format="yyyy-MM-dd" clearable v-model="form.expectedPurchaseDate"
          type="date"></el-date-picker>
      </el-form-item>
      <el-form-item :label="$t('purchase.isEmergencyPurchase')" prop="isEmergencyPurchase">
        <el-select v-model="form.isEmergencyPurchase" clearable>
          <el-option v-for="item in isEmergencyPurchaseList" :value="item.value" :key="item.value"
            :label="item.label"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :label="$t('purchase.prType')" prop="prType">
        <el-select v-model="form.prType" clearable >
          <el-option v-for="item in prTypeList" :value="item.dictKey" :key="item.dictKey"
      ...
      <el-form-item label-width="0" style="width: 100%;">
        <div class="fr flex-center mb10" v-if="!disabled">
          ...
          <el-button @click="downloadFile">{{ $t('purchaseDetail.downloadTemplate') }}</el-button>
          <upload @handleRequest="handleRequestExport" :showTip="false" class="mx10"
            url="/api/oa-portal/purchaseApply/getPurchaseDetailByFile"></upload>
        </div>
        <div style="color: red;">{{ $t('purchaseDetail.notice') }}</div>
        <el-table :class="{ 'w-720': isPdfDown, 'w-100': !isPdfDown }" ref="multipleTable" :data="form.purchaseDetailList"
          ...
          <el-table-column label="相关项目" :width="isPdfDown ? 50 : 320">
            ...
            <template slot-scope="scope">
              <el-form-item :prop="'purchaseDetailList.' + scope.$index + '.projectNo'" :rules="rules.projectNo">
                <el-select v-if="!disabled" v-model="scope.row.projectNo" filterable clearable
                  @change="getProjectRow(scope.row, scope.$index)">
                  <el-option v-for="item in projectList" :key="item.projectNo" :label="item.projectName"
          ...
          <el-table-column :label="$t('purchaseDetail.columns.sorpTime')" :width="isPdfDown ? 50 : 180">
            <template slot-scope="scope">
              <el-form-item>
                <el-date-picker v-if="!disabled" style="width: 100%" value-format="yyyy-MM-dd" clearable
                  v-model="scope.row.sorpTime" type="date"></el-date-picker>
          ...
          <el-table-column label="预算科目" :width="isPdfDown ? 50 : 280">
            ...
            <template slot-scope="scope">
              <el-form-item :prop="'purchaseDetailList.' + scope.$index + '.subjectId'" :rules="rules.subjectId">
                <elSelectTree v-if="!disabled" @getValue="getSubjetRow($event, scope.row, scope.$index)"
                  :value="{ name: scope.row.subjectName }" :options="subjectList" :accordion="false" :disabled="disabled"
          ...
          <el-table-column label="成本中心" :width="isPdfDown ? 50 : 200">
            ...
            <template slot-scope="scope">
              <el-form-item :prop="'purchaseDetailList.' + scope.$index + '.costCenterName'"
                :rules="rules.costCenterName">
                <el-select v-if="!disabled" v-model="scope.row.costCenterName"
                  @change="getCostItem(scope.row, scope.$index)" :disabled="disabled">
          ...
          <el-table-column :label="$t('purchaseDetail.columns.subjectCode')" :width="isPdfDown ? 50 : 180">
            <template slot-scope="scope">
              <el-form-item>
                <el-input v-if="!disabled" v-model="scope.row.subjectCode"></el-input>
                <div v-else>{{ scope.row.subjectCode }}</div>
              </el-form-item>
            </template>
          </el-table-column>
          <el-table-column :label="$t('purchaseDetail.columns.budgetInfo')" :width="isPdfDown ? 60 : 120">
            <template slot-scope="scope">
              <div v-html="scope.row.budgetInfo"></div>
            </template>
          </el-table-column>
          <el-table-column :label="$t('purchaseDetail.columns.productName')" :width="isPdfDown ? 50 : 120">
            <template slot="header">
              {{ $t('purchaseDetail.columns.productName') }}
              <span class="red">*</span>
            </template>
            <template slot-scope="scope">
              <el-form-item :prop="'purchaseDetailList.' + scope.$index + '.partsName'" :rules="rules.partsName">
                <el-input v-if="!disabled" v-model="scope.row.partsName"></el-input>
                <div v-else>{{ scope.row.partsName }}</div>
              </el-form-item>
            </template>
          </el-table-column>
          <el-table-column :label="$t('purchaseDetail.columns.techSpec')" :width="isPdfDown ? 50 : 120">
            <template slot-scope="scope">
              <el-form-item>
          ...
          <el-table-column label="单位" :width="isPdfDown ? 50 : 120">
            ...
            <template slot-scope="scope">
              <el-form-item :prop="'purchaseDetailList.' + scope.$index + '.unitName'" :rules="rules.unitName">
                <el-input v-if="!disabled" v-model="scope.row.unitName"></el-input>
          ...
          <el-table-column label="数量" :width="isPdfDown ? 50 : 120">
            ...
            <template slot-scope="scope">
              <el-form-item :prop="'purchaseDetailList.' + scope.$index + '.quantity'" :rules="rules.quantity">
                <el-input @blur="dealQuantity(scope.row)" v-if="!disabled" v-model="scope.row.quantity"></el-input>
          ...
          <el-table-column label="单价(含税)" :width="isPdfDown ? 50 : 120">
            <template slot-scope="scope">
              <el-form-item :prop="'purchaseDetailList.' + scope.$index + '.unitPrice'" >
          ...
          <el-table-column label="总价格(原币)" :width="isPdfDown ? 50 : 120">
            ...
            <template slot-scope="scope">
              <el-form-item :prop="'purchaseDetailList.' + scope.$index + '.totalPriceOriginal'"
                :rules="rules.totalPriceOriginal">
                <el-input @change="getOriginalAmountToTal()" v-if="!disabled"
                  v-model="scope.row.totalPriceOriginal"></el-input>
                <div v-else>{{ scope.row.totalPriceOriginal }}</div>
              </el-form-item>
            </template>
          </el-table-column>
          <el-table-column :label="$t('purchaseDetail.columns.totalPriceRMB')" :width="isPdfDown ? 60 : 120">
            <template slot-scope="scope">
              <el-form-item>
      ...
      <el-form-item label-width="0" style="width: 100%;">
        ...
        <el-table :class="{ 'w-720': isPdfDown, 'w-100': !isPdfDown }" ref="multipleTable"
          :data="form.purchaseSupplierList" @selection-change="receiveDetailSelectionChange" class="mb20 table-now">
          ...
          <el-table-column :label="$t('purchaseApplication.supplierList')">
            ...
            <template slot-scope="scope">
              <el-form-item :prop="'purchaseSupplierList.' + scope.$index + '.supplierName'" :rules="rules.supplierName">
                <el-input v-if="!disabled" v-model="scope.row.supplierName"></el-input>
          ...
          <el-table-column :label="$t('purchaseApplication.contact')">
            ...
            <template slot-scope="scope">
              <el-form-item :prop="'purchaseSupplierList.' + scope.$index + '.contactName'" :rules="rules.contactName">
                <el-input v-if="!disabled" v-model="scope.row.contactName"></el-input>
          ...
          <el-table-column :label="$t('purchaseApplication.phone')">
            ...
            <template slot-scope="scope">
              <el-form-item :prop="'purchaseSupplierList.' + scope.$index + '.contactTel'" :rules="rules.contactTel">
                <el-input v-if="!disabled" v-model="scope.row.contactTel"></el-input>
          ...
          <el-table-column :label="$t('purchaseApplication.email')">
            ...
            <template slot-scope="scope">
              <el-form-item :prop="'purchaseSupplierList.' + scope.$index + '.contactEmail'" :rules="rules.contactTel">
                <el-input v-if="!disabled" v-model="scope.row.contactEmail"></el-input>
          ...
          <el-table-column :label="$t('purchaseApplication.relatedCases')">
            ...
            <template slot-scope="scope">
              <el-form-item :prop="'purchaseSupplierList.' + scope.$index + '.remark'" :rules="rules.remark">
                <el-input v-if="!disabled" v-model="scope.row.remark"></el-input>
                <div v-else>{{ scope.row.remark }}</div>
              </el-form-item>
            </template>
      ...
      <el-form-item label-width="0" style="width: 100%;">
        ...
        <el-table :class="{ 'w-720': isPdfDown, 'w-100': !isPdfDown }" ref="multipleTable"
          :data="form.purchaseTechEvalList" @selection-change="custContactSelectionChange" class="mb20 table-now">
          <el-table-column type="selection" width="42"></el-table-column>
          <el-table-column label="公司名称">
            ...
            <template slot-scope="scope">
              <el-form-item :prop="'purchaseTechEvalList.' + scope.$index + '.supplierName'" :rules="rules.supplierName">
                <el-input v-if="!disabled" v-model="scope.row.supplierName"></el-input>
                <div v-else>{{ scope.row.supplierName }}</div>
              </el-form-item>
            </template>
          ...
          <el-table-column label="报价日期">
            ...
            <template slot-scope="scope">
              <el-form-item :prop="'purchaseTechEvalList.' + scope.$index + '.quotationDate'"
                :rules="rules.quotationDate">
                <el-date-picker v-if="!disabled" style="width: 100%" value-format="yyyy-MM-dd" clearable
                  v-model="scope.row.quotationDate" type="date"></el-date-picker>
          ...
          <el-table-column label="报价价格（原币）">
            ...
            <template slot-scope="scope">
              <el-form-item :prop="'purchaseTechEvalList.' + scope.$index + '.quotationPriceOriginal'"
                :rules="rules.quotationPriceOriginal">
                <el-input v-if="!disabled" v-model="scope.row.quotationPriceOriginal"></el-input>
          ...
          <el-table-column :label="$t('purchaseApplication.quotationPriceRMB')">
            <template slot-scope="scope">
              <el-form-item>
                {{ getRMBAmount(scope.row.quotationPriceOriginal, scope, 'quotationPriceRmb') }}
              </el-form-item>
            </template>
          </el-table-column>
          <el-table-column label="评定结果">
            ...
            <template slot-scope="scope">
              <el-form-item :prop="'purchaseTechEvalList.' + scope.$index + '.evalResult'" :rules="rules.evalResult">
                <el-select v-if="!disabled" v-model="scope.row.evalResult" filterable clearable>
                  <el-option v-for="item in evalResultList" :key="item.value" :label="item.label"
          ...
          <el-table-column label="合格/不合格说明">
            ...
            <template slot-scope="scope">
              <el-form-item :prop="'purchaseTechEvalList.' + scope.$index + '.remark'" :rules="rules.remark">
                <el-input v-if="!disabled" v-model="scope.row.remark"></el-input>
                <div v-else>{{ scope.row.remark }}</div>
              </el-form-item>
            </template>
          ...
          <el-table-column label="评定得分">
            ...
            <template slot-scope="scope">
              <el-form-item :prop="'purchaseTechEvalList.' + scope.$index + '.evalScore'" :rules="rules.evalScore">
                <el-input v-if="!disabled" v-model="scope.row.evalScore"></el-input>
      ...
      <el-form-item label="（盖章）报价单" label-width="140px" prop="quotationSheetList" style="width: 100%;">
        <upload :isHistory="isHistory" :fileList="form.quotationSheetList" btnName="上传"
          @handleRequest="handleQuotationSheet" :multiple="true" :showFileList="true"></upload>  
      </el-form-item>
      <el-form-item label="SOR/SOW" label-width="140px" prop="sorOrSowList" style="width: 100%;">
        <upload :isHistory="isHistory" :fileList="form.sorOrSowList" btnName="上传"
          @handleRequest="handleSOROrSOW" :multiple="true" :showFileList="true"></upload>
      </el-form-item>
    </el-form>
  </basic-container>
</template>
...
<script>
import { getFormatDate, numberToTraditionalChinese, downBlobFile } from "@/util/util";
import PurchaseApplyApi from "@/api/bomPurchaseApply/index";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        titleName: [
          { required: true, message: this.$t('purchase.pleaseEnterTitle'), trigger: 'change' }
        ],
        currency: [
          { required: true, message: this.$t('purchase.pleaseSelectCurrency'), trigger: 'change' }
        ],
        exchangeRate: [
          { required: true, message: this.$t('purchase.pleaseEnterExchangeRate'), trigger: 'change' },
          { validator: validatenNum }
        ],
        applyDesc: [
          { required: true, message: this.$t('purchase.pleaseEnterPurchaseDesc'), trigger: 'change' }
        ],
        purchaseType: [
          { required: true, message: this.$t('purchase.pleaseSelectPurchaseType'), trigger: 'change' }
        ],
        expectedPurchaseDate: [
          { required: true, message: this.$t('purchase.pleaseSelectExpectedDate'), trigger: 'change' }
        ],
        isEmergencyPurchase: [
          { required: true, message: this.$t('purchase.pleaseSelectEmergency'), trigger: 'change' }
        ],
        projectNo: [
          { required: true, message: this.$t('purchase.pleaseEnterProject'), trigger: 'change' }
        ],
        projectManageId: [
          { required: true, message: this.$t('purchase.pleaseEnterProjectManager'), trigger: 'change' }
        ],
        costCenterName: [
          { required: true, message: this.$t('purchase.pleaseEnterCostCenter'), trigger: 'change' }
        ],
        subjectId: [
          { required: true, message: this.$t('purchase.pleaseEnterBudgetSubject'), trigger: 'change' }
        ],
        supplierName: [
        ...
        evalScore: [
          { required: true, message: this.$t('purchase.pleaseEnter'), trigger: 'blur' },
          { validator: validatenNum }
        ],
        quotationDate: [
          { required: true, message: this.$t('purchase.pleaseEnter'), trigger: 'change' }
  ...
  methods: {
    async getDetail() {
      let res = await PurchaseApplyApi.details({ businessId: this.businessId, purchaseApplyNo: this.purchaseApplyNo });
      if (res.data.code === 200) {
        this.form = res.data.data;
        this.form.purchaseType = this.form.purchaseType ? this.form.purchaseType.toString() : ""
        ...
        this.form.purchaseDetailList.map((ele, index) => {
          ele.projectManageId = ele.projectManageId ? ele.projectManageId.toString() : ''
    ...
    getProjectRow(row, index) {
      ...
      row.projectManageId = obj.projectManageId ? obj.projectManageId.toString() : null
    ...
    getOriginalAmountToTal(listKey = 'purchaseDetailList', amountKey = 'totalPriceOriginal', formKey = 'totalAmountOriginal') {
      if (this.form['totalAmountRmb']) {
        this.deepTitle = `(合计金额：${Number(this.form['totalAmountRmb']).toFixed(2)})`
      }
      this.form[listKey] = this.form[listKey] || []
      let arr = this.form[listKey].map(ele => ele[amountKey])
      var sum = this.form[listKey].length === 0 ? 0 : arr.reduce(function (prev, curr, idx, arr) {
        return Number(prev || 0) + Number(curr || 0);
      });
      this.form[formKey] = sum
      this.form['totalAmountRmb'] = Number((sum || 0) * (this.form.exchangeRate || 1)).toFixed(2)
      this.form.totalAmountRmbUpper = numberToTraditionalChinese(this.form.totalAmountRmb)
      this.form.totalAmountOriginalUpper = numberToTraditionalChinese(this.form.totalAmountOriginal)
      let newTitle = `(合计金额：${Number(this.form['totalAmountRmb']).toFixed(2)})`
      if (!this.form.titleName.includes('合计金额')) {
        this.form.titleName = `${this.form.titleName}${newTitle}`
      } else {
        this.form.titleName = this.form.titleName.replace(this.deepTitle, newTitle)
      }
      if (!isNaN(sum)) {
    ...
    groupedByCode() {
      return this.form.purchaseDetailList.reduce((acc, item) => {
        acc[item.subjectCode + item.costCenterId] = acc[item.subjectCode + item.costCenterId] || []
    ...
    testMoney() {
      ...
      for (let key in arrGroup) {
        if (arrGroup[key].length > 0) {
          ...
          let arrMoney = arrGroup[key].map(ele => Number(ele.totalPriceOriginal))
    ...
    copyChoose() {
      ...
      let arr = JSON.parse(JSON.stringify(this.saleContractDetailMultipleSelection))
      arr.map((ele, index) => {
        ele.key = Date.now() + index
    ...
    deleteChooseRow() {
      ...
      this.form.purchaseTechEvalList = this.form.purchaseTechEvalList.filter((ele) => {
        return !keys.includes(ele.key) || !ids.includes(ele.id);
      });
    },
    async getDepartment() {
