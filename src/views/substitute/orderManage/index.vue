  ...
  <basic-container>
    ...
    >
      ...
      <template slot-scope="{ type, size, row }" slot="menu">
        <!-- <el-button
          v-if="!isFinish(row) && !isCancel(row) && !isPlanor && row.orderNo"
          icon="el-icon-circle-close"
          :size="size"
          :type="type"
          @click="rowDel(row)"
          >{{ $t("orderManage.cancel") }}</el-button
        >
    -->
        <el-button
          v-if="row.orderStatus===0&&!row.orderNo"
          icon="el-icon-view"
          :size="size"
          :type="type"
          @click="editRow(row, 'view')"
          >{{ $t("orderManage.change") }}</el-button
        >
      </template>
    </avue-crud>
  </basic-container>
...
export default {
  ...
  methods: {
    ...
    handleExport() {
      this.$confirm(
        this.$t("orderManage.exportConfirm"),
        this.$t("orderManage.exportConfirm"),
        {
          confirmButtonText: this.$t("orderManage.confirm"),
          cancelButtonText: this.$t("orderManage.cancel"),
    ...
    onLoad(page, params = {}) {
      ...
      substituteOrderManageApi.selectPage(params).then((res) => {
