    ...
    >
      <el-row class="row-flex">
        <el-col :span="12" v-if="type === 'see'">
          <el-form-item :label="$t('stock.stockInNo')">
            <el-input v-model="ruleForm.stockInNo" disabled></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item :label="$t('stock.partsNo')" prop="partsId">
            ...
            >
              <el-option
                v-for="item in options.partsMaintListList"
                :key="item.id"
                :label="item.partsNo"
                :value="item.id"
              ></el-option>
        ...
        <el-col :span="12" v-if="type === 'see'">
          <el-form-item :label="$t('stock.supplier')" prop="">
            <el-select v-model="ruleForm.supplier" disabled>
              <el-option
                v-for="item in options.partsMaintListList"
                :key="item.id"
                :label="item.supplier"
        ...
        <el-col :span="12">
          <el-form-item :label="$t('stock.partsTypeStock')" prop="partsType">
            <el-select v-model="ruleForm.partsType" :disabled="type === 'see'">
              <el-option label="供应商免费件" value="供应商免费件"></el-option>
              <el-option label="供应商采购件" value="供应商采购件"></el-option>
              <el-option label="补交" value="补交"></el-option>
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="12">
      ...
      <el-form-item class="text-r">
        <el-button type="primary" @click="saveRemarkOrFile('ruleForm')">
          保存备注/附件</el-button
        >
        <el-button
          v-if="!ruleForm.payStatus || ruleForm.payStatus === '1'"
          type="primary"
...
export default {
  ...
  methods: {
    ...
    async getTableList() {
      ...
      this.ruleForm.partsNo = row.partsNo
    ...
    getDetail(id) {
      detail({ id }).then((res) => {
        if (res.data.code === 0) {
          ...
          this.ruleForm.supplier = this.row.supplier;
          this.partsChange(this.ruleForm.partsId);
        }
      });
    },
    async saveRemarkOrFile() {
      let res = await updateStockInRemarkOrFile({
        id: this.ruleForm.id,
          ...
          saveOrUpdate(this.ruleForm).then((res) => {
            if (res.data.code === 0) {
              ...
              if (res.data.message&&res.data.message!=="操作成功") {
                this.$message.warning(res.data.message, { duration: 5000 });
              }
