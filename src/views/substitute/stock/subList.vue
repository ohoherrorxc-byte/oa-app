  ...
  <basic-container>
    <SearchForm :searchForm="searchForm" @handleSubmit="handleSubmit">
      <template #operation="{ row }">
        <el-button
          @click="handleOpenDialog(row, 'see')"
          type="text"
          icon="el-icon-view"
          >{{ $t("commonContent.see") }}</el-button
        >
        <el-button
          v-if="row.status === 1"
    ...
    >
      ...
      <el-table-column
        prop="partsNo"
        :label="$t('stock.partsNo')"
      ></el-table-column>
      <el-table-column
        prop="partsName"
        :label="$t('stock.partsName')"
        show-overflow-tooltip
        width="130"
      ></el-table-column>
      <el-table-column
        prop="parentFunctionName"
        :label="$t('stock.partsType')"
      ></el-table-column>
      <el-table-column
        prop="stockInNo"
        :label="$t('stock.stockInNo')"
      ></el-table-column>
      <el-table-column
        prop="storeName"
        :label="$t('stock.storeName')"
      ></el-table-column>
      <el-table-column
        prop="intoTime"
        :label="$t('stock.intoTime')"
      ></el-table-column>
      <el-table-column
        prop="goodsShelvesNo"
      ...
      <el-table-column
        prop="stockNumber"
        :label="$t('stock.stockNumber')"
      ></el-table-column>
      <el-table-column
        prop="unitPriceCny"
        :label="$t('stock.unitPriceCny')"
      ></el-table-column>
      <el-table-column
        prop="totalStockNumber"
        :label="$t('stock.totalStockNumber')"
      ></el-table-column>
      <el-table-column
        prop="totalPriceCny"
        :label="$t('stock.totalPriceCny')"
      ></el-table-column>
    </el-table>
...
import { selectPage } from "@/api/substitute/subStock";
export default {
  components: {
    subOutStock,
  },
            ...
            onClick: () => {
              if (!this.partsNos)
                return this.$message.warning(
                  this.$t("commonContent.pleaseSelectAtLeastOneStock")
                );
              this.outStock = true;
            },
          },
