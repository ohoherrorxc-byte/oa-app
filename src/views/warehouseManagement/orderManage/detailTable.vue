<template>
    <div>
        <el-form>
            <div v-for="item in tableData" class="">
                <el-row>
                    <el-col span="8"> <el-form-item label="零件号">{{ item.partsNumber }}</el-form-item></el-col>
                    <el-col span="8"> <el-form-item label="零件名称">{{ item.partsName }}</el-form-item></el-col>
                    <el-col span="8"> <el-form-item label="要求到货日期">{{ item.needDate | filter_date }}</el-form-item></el-col>
                    <el-col span="8"> <el-form-item label="需求数量">{{ item.needNumber }}</el-form-item></el-col>
                    <el-col span="8"> <el-form-item label="项目名称">{{ item.needNumber }}</el-form-item></el-col>
                    <el-col span="8"> <el-form-item label="阶段">{{ item.stage }}</el-form-item></el-col>
                </el-row>
                <el-form-item style="width: 100%;">
                    <el-table :data="item.deliverList" style="width: 100%">
                        <el-table-column fixed prop="date" label="计划交付日期" width="140">
                            <template slot-scope="scope">
                                <el-date-picker v-if="!isApprove(scope.row)" style="width: 100%"
                                    value-format="yyyy-MM-dd HH:mm:ss" clearable v-model="scope.row.planDeliverDate"
                        ...
                        <el-table-column prop="name" label="计划交付数量">
                            <template slot-scope="scope">
                                <el-input v-if="!isApprove(scope.row)" v-model="scope.row.deliverNumber"></el-input>
                                <div v-else>{{ scope.row.deliverNumber }}</div>
                            </template>
                        </el-table-column>
                        <el-table-column prop="province" label="状态">
                            <template slot-scope="scope">
                                <!-- <el-input v-if="!isApprove(scope.row)" v-model="scope.row.deliverStatus"></el-input> -->
                                <div>{{ statusObj[scope.row.deliverStatus] }}</div>
                            </template>
                        </el-table-column>
                        <el-table-column label="审核日期">
                            <template slot-scope="scope">
                                <!-- <el-input v-if="!isApprove(scope.row)" v-model="scope.row.planApplyDate"></el-input> -->
                    ...
                    7:上传入场单（管理员操作）；8：可交货（计划员操作）；9.签收审核（管理员操作）-->
                            ...
                            <template slot-scope="scope">
                                <div>
                                    ...
                                    <!-- <el-button type="text" style="color: red;"
                                        v-if="scope.row && scope.row.deliverStatus === 2">计划审核拒绝</el-button> -->
                                    <!-- <el-button type="text" style="color: red;"
                                        v-if="scope.row && scope.row.deliverStatus === 5">交付审核拒绝</el-button> -->
                                    <el-button
                        ...
                        <el-table-column label="操作">
                            <template slot-scope="scope">
                                <i v-if="scope.$index === 0" @click="addPayList(item, scope.row)"
                                    class="el-icon-circle-plus-outline"></i>
                                <i v-if="scope.row && scope.row.deliverStatus === 0" @click="delPayList(item, scope.$index)"
        ...
        <el-dialog :title="titleStatus" :visible.sync="dialogFormVisible" :append-to-body="true" class="fileStyle">
            <el-form :model="form">
                <el-form-item label="附件">
                    <upload :fileList="form.fileList" ref="uploadList" :disabled="disableFile" btnName="上传"
                        @handleRequest="handleRequest" :multiple="true" :showFileList="true"></upload>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer" v-if="!seeView">
                <el-button @click="dialogFormVisible = false" v-if="curItem.deliverStatus === 3">取 消</el-button>
                <el-button type="primary" @click="submit(4,9)"
                    v-if="curItem.deliverStatus === 3 || curItem.deliverStatus === 5 || curItem.deliverStatus === 8||curItem.deliverStatus === 10">提交</el-button>
                <el-button type="primary" @click="submit(8)" v-if="curItem.deliverStatus === 7">提交</el-button>
                <el-button type="primary" @click="submit(5, 10)"
                    v-if="curItem.deliverStatus === 4 || curItem.deliverStatus === 9">拒绝</el-button>
                <el-button type="primary" @click="submit(7, 6)"
                    v-if="curItem.deliverStatus === 4 || curItem.deliverStatus === 9">同意</el-button>
            </div>
        </el-dialog>
    </div>
</template>
...
export default {
    ...
    computed: {
        ...
        disableFile() {
            return this.curItem.deliverStatus !== 3 && this.curItem.deliverStatus !== 7 && this.curItem.deliverStatus !== 8 && this.curItem.deliverStatus !== 5&&this.curItem.deliverStatus !== 10
        }
    ...
    methods: {
        ...
        openDialog(subItem, item, seeView) {
            ...
            } else if (this.curItem.deliverStatus === 8||this.curItem.deliverStatus === 10) {
        ...
        async submit(key, key2) {
            let obj = {
                ...
                ...this.curItem,
                id: this.curItem.id,
                deliverStatus: (this.curItem.deliverStatus === 8||this.curItem.deliverStatus === 9||this.curItem.deliverStatus === 10) ? key2 : key,
