...
function findTagContains(state, value) {
  let tag, key;
...
const navs = {
  ...
  mutations: {
    ADD_TAG: (state, action) => {
      ...
      /*let tag = findTagContains(state, "vehiclePrepaidCardEdit").tag;
      state.tagList = state.tagList.filter(item => {
        return !diff(item, tag);
      })*/
      action.value = action.value.split('?')[0];
