...
import {indexUrl} from "@/config/env"; // vuex
Vue.use(VueRouter)
//创建路由
...
export function getLoginRouter(){
  if (Store.state.user.isFleet)
    return website.loginRouterFleet;
  else if (Store.state.user.isSaic)
    return website.loginRouterSaic;
  else
    return website.loginRouter;
}
