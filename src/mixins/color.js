...
export default function () {
  return {
    ...
    created() {
      this.updateTheme(this.colorName, this.themeVal);
    },
    methods: {
      updateTheme(val, oldVal) {
        ...
        if (!this.chalk) {
          const url = `${baseUrl}/cdn/element-ui/${version}/theme-chalk/index.css`;
        ...
        for (let i = 0; i < link.length; i++) {
          const style = link[i];
          if (style.href.includes('css') && !style.href.includes('theme-chalk')) {
